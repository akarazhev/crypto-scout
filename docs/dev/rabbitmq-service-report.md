...
- **Queues**:
    - `metrics-cmc-queue`: For CoinMarketCap fear and greed index metrics
    - `metrics-bybit-queue`: For Bybit launch pool metrics
...
- **Bindings**:
    - `metrics-exchange` → `metrics-cmc-queue` with routing key `metrics.cmc`
    - `metrics-exchange` → `metrics-bybit-queue` with routing key `metrics.bybit`
