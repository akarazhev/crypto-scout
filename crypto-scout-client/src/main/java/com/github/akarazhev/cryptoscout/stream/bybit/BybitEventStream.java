...
@Service
@Qualifier("bybitEventStream")
final class BybitEventStream implements DataStream {
    private static final Logger LOGGER = LoggerFactory.getLogger(BybitEventStream.class);
    private final BybitDataSupplier bybitDataSupplier;

    public BybitEventStream(final BybitDataSupplier bybitDataSupplier) {
        this.bybitDataSupplier = bybitDataSupplier;
    }

    @Override
    public Flowable<Payload<Map<String, Object>>> stream() {
        return bybitDataSupplier.events()
